...
# IA migrada de Google Gemini para Anthropic Claude

import os
import pytz
from datetime import datetime
from dotenv import load_dotenv
import anthropic  # Substitui: from google import genai

from modules.calculadora import realizar_orcamento, exibir_resumo

load_dotenv()

chave = os.getenv("ANTHROPIC_API_KEY")  # Substitui: GOOGLE_API_KEY
...
def chamar_ia_pink_chat(perguntar_usuario):
    """
    Chama a API da Anthropic (Claude) no lugar do Google Gemini.
    O comportamento e o prompt do assistente foram mantidos idênticos.
    """
    if not chave:
    ...
    try:
        ...
        response = client.messages.create(
            model="claude-haiku-4-5-20251001",  # Modelo rápido e econômico — troque por claude-sonnet-4-6 se preferir
            max_tokens=512,
            system=system_prompt,
            messages=[
                {"role": "user", "content": perguntar_usuario}
            ]
        )
...
if __name__ == "__main__":
    while True:
        ...
        # FALLBACK PARA INTELIGÊNCIA ARTIFICIAL (Claude)
        else:
